...
import { createColor, getColors } from "@/services/admin/colors";
...
export default function Colors() {
    ...
    useEffect(() => {
        if (search.length > 0) {
            const dataFiltered = colors.filter((item) => {
                return Object.values(item)
                    .join(" ")
                    .toLowerCase()
                    .includes(search.toLowerCase());
            })
            setFilteredColors(dataFiltered);
        } else {
    ...
    return (
        <>
            ...
            <SidebarProvider>
                <AppSidebar active={"Colors"} user={isLogin} />
                <SidebarInset className={`bg-[#212121] text-white`}>
                    ...
                    <div className="container mx-auto px-10">
                        <Button className={"bg-blue-600 hover:bg-blue-900 cursor-pointer rounded-md p-2"} onClick={() => setIsOpen(true)} >Añadir Color</Button>
                        <DataTable
                            title={<h1 className="text-4xl my-4 text-center font-bold">Colores</h1>}
                            columns={columns}
                            data={filteredColors}
                            pagination
                            responsive
                            ...
                            <DialogContent className={`bg-[#353535] text-white border-none overflow-y-auto max-h-200`}>
                                <form onSubmit={handleSubmit(onSubmit)}>
                                    <DialogHeader>
                                        ...
                                        <DialogDescription>Verifica bien el color.</DialogDescription>
                                    </DialogHeader>
                                    <div className="mt-5">
                                        <InputFieldModal type="text" error={errors.color?.message} label={"Color"} {...register("color", {
